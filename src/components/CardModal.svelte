<script>
  ...
  let store
  onMount(()=> {
    const unsubscribe = cardStore.subscribe(async (value) => {
      store = value
    })
  })
...
{#if store && store.visible}
	...
	{:then nothing}
	  ...
	  >
	    <div class="w-3/4 bg-indigo-500 py-8 px-4 pb-10 text-white">
	      <button
	        class="w-100 float-right font-bold -mt-6"
	        id="btnClose">
	          x
	        </button>
	      <div class="flex flex-row items-start justify-start items-start">
	        ...
	        <div class="w-1/5 bg-red-3000 pl-2">
	          ...
	          <button class="w-full bg-indigo-600 block p-2 my-2">
	            ...
	            <!-- <span class="float-left ml-2">Attachment</span> -->
